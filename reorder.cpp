...
namespace Reorder 
{
    ...
    void build_NN(const char* const transposed_matrix, DistanceMatrix * DISTANCE_MATRIX, const unsigned SUBSAMPLED_ROWS, const unsigned OFFSET, std::vector<int>& order)
    {
        ...
        std::vector<int> path = {firstVertex};
        ...
        DistanceFunctions df = VPTree<int>::bindDistanceFunctions(
            [=, &counter, t = transposed_matrix](int a, int b) -> double {
                ++counter;
                return columns_hamming_distance(transposed_matrix, SUBSAMPLED_ROWS, a+OFFSET, b+OFFSET);
            },
            [&DISTANCE_MATRIX](int a, int b) -> double { return DISTANCE_MATRIX->get(a, b); },
            [&DISTANCE_MATRIX](int a, int b, double d) { DISTANCE_MATRIX->set(a, b, d); }
        );
    ...
    void build_double_end_NN(const char* const  transposed_matrix, DistanceMatrix * DISTANCE_MATRIX, const unsigned SUBSAMPLED_ROWS, const unsigned OFFSET, std::vector<int>& order)
    {
        ...
        //Deque for building path with first vertex as starting point
        std::deque<int> orderDeque = {firstVertex};
    ...
    const char* const get_transposed_matrix(const char * const MAPPED_FILE, const unsigned HEADER, const unsigned COLUMNS, const unsigned ROW_LENGTH, const unsigned NB_GROUPS, const unsigned GROUPSIZE, const unsigned long SUBSAMPLED_ROWS, std::vector<DistanceMatrix*>& distanceMatrices)
    {
        ...
        /**
         * Hamming distance can be computed like this: dH(A,B) = H(A ^ B) ;; with ^ as XOR symbol
         * 
         * Distance matrices are coded as an 1D array, where rows are concatenated
         * To retrieve the distance between A and B, we can retrieve it with matrix[A*row_size + B] or matrix[B*row_size + A]
         * (upper and lower triangle are symmetrical)
         * 
         * MAX_ROW/8 is the size of a row in the transposed buffer (as rows are now columns, so the width of transposed matrix) ;; /8 to get width in bytes (=in size of char) instead of bits
         * groupSize*m is added to i in order to get the start of a group of columns
         */
    ...
    //Some columns may not be used and they need to stay at the same location
    ...
    void launch(const std::string& MATRIX, const unsigned SAMPLES, const unsigned HEADER, const unsigned GROUPSIZE, unsigned subsampled_rows)
    {
        ...
        /*int fdorder = open("order.bin", O_WRONLY | O_CREAT | O_TRUNC, 0644);
        write(fdorder, reinterpret_cast<const char*>(order.data()), sizeof(int)*order.size());
        close(fdorder);*/
...
int main(int argc, char ** argv)
{
    if(argc != 6)
    {
        std::cout << "Usage: reorder <matrix> <samples> <header> <groupsize> <subsampled_rows>\n\n";
        return 1;
    }
    ...
    Reorder::launch(matrix, samples, header, groupsize, subsampled_rows);
}
